...
public sealed class CsrfMiddleware(RequestDelegate next)
{
    ...
    ///
    /// The SignalR hub is exempt too, for a different reason: negotiate is a POST, but this
    /// hub only ever pushes ticks server-to-client — there is no client-invoked mutation for
    /// a forged request to trigger, so there is nothing here for CSRF to protect. It is also
    ...
    private static readonly string[] ExemptPaths =
        ["/api/v1/auth/login", "/api/v1/auth/register", "/hubs"];

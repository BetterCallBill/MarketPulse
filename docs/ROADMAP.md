...
**Verified against commit `4b5be35` on `feature/slice-8-observability`, 2026-08-06**
...
| 5 · Cloud & pipeline | **Partial** | CI runs backend tests, frontend tests, and Playwright E2E against a real database; Serilog structured logging + OpenTelemetry traces and metrics in both hosts (`marketpulse-api`, `marketpulse-alerts`) exported to the Aspire dashboard, nine `MarketPulse` custom instruments, `traceparent` propagated across RabbitMQ by hand, the transient-redelivery loop bounded (ADR-009 amended), real `/health/ready` (SQL Server + RabbitMQ), browser-minted `X-Correlation-Id`; the full Terraform surface under `infra/` (VPC without NAT, ALB, ECS Fargate ×2, ECR ×2, RDS SQL Server Express, Amazon MQ RabbitMQ, S3 + dual-origin CloudFront), `docs/cost-model.md`, ADR-011, CI `fmt`/`validate` gate — the live `terraform apply` rehearsal itself is pending refreshed AWS credentials | Lambda snapshot, deployment pipeline, the live apply rehearsal |
...
One phase untouched, four done, one partial.
...
| 8 · Observability | 2026-08-06 | Serilog structured JSON logging + OpenTelemetry traces and metrics wired into both hosts (`marketpulse-api`, `marketpulse-alerts`), exported via OTLP to the Aspire dashboard (compose service `aspire-dashboard`, UI `:18888`, OTLP `:4317`); nine `MarketPulse` meter instruments on the hot paths — tick persistence, buffer drops, alert evaluation/triggering, notification redeliveries/dead-letters, and auth security events (failed logins, lockouts, refresh-token reuse); W3C `traceparent` propagated across RabbitMQ by hand (`MessagingTelemetry`, an explicit `CompositeTextMapPropagator` rather than the ambient default) so a trace continues from producer to consumer across the broker; the 4a/4b carried-over unbounded transient-requeue loop finally bounded (`RabbitMqOptions.RetryLimit`, default 5; `TransientRetry.RetryOrDeadLetterAsync`; publisher confirms enabled on the retry-republish channel) and ADR-009 amended to record it; `GET /health/ready` (SQL Server + RabbitMQ checks) added alongside the untouched liveness `/health`; a browser-minted `X-Correlation-Id` on every API client request. Task 10's manual rehearsal (2026-08-06, `MarketData:Source=Fake`, both hosts run by hand against compose) confirmed: one real distributed trace spanning `marketpulse-alerts` (outbox publish) → the broker → `marketpulse-api` (consume) → SQL Server (the `INSERT`), 3 spans/29.48ms, visually confirmed in the Aspire dashboard UI via a throwaway Playwright driver; the nine-instrument climb under live `FakeTickService` traffic (`marketpulse.ticks.persisted` ~9k→15k over 5 minutes, `marketpulse.alerts.evaluated`/`.triggered` moving on the worker); and the forced-failure leg (`RabbitMq__RetryLimit=2`, SQL container stopped) landed a message in `api.notifications.dlq` with zero loss — but not through the intended redelivery path: a full connection outage throws an unwrapped `SqlException`, not the `DbUpdateException` `AlertTriggeredConsumer`'s classification catches, so the message fell straight to the generic catch and dead-lettered on the first attempt, without a single redelivery warning or a `marketpulse.notifications.redeliveries`/`.dead_letters` increment. Recorded honestly as a real gap in the classification's fault coverage rather than silently patched — see the 4a-findings note below and task-10-report.md. The rehearsal also found the happy path itself under-logs: a successful consume emits no Information-level correlation log, only the warn/error paths (a redelivery, a dead-letter) log anything, so tracing a message that worked end-to-end depends on the distributed trace rather than the log stream. Closes the observability half of phase 5; IaC (9) and the deployment pipeline (10) remain |
| 9 · Infrastructure as code | 2026-08-07 | The full Terraform surface under `infra/`: a `bootstrap/` root (versioned, encrypted S3 state bucket; the main root backs onto it with S3-native `use_lockfile` locking — no DynamoDB table) and the main root — VPC across 2 AZs with **no NAT gateway** (ECS tasks in public subnets behind strict security groups, the ALB admitting port 80 only from CloudFront's origin-facing prefix list; RDS and the broker in no-internet private subnets), ALB health-checked on slice 8's liveness `/health`, ECS cluster with `marketpulse-api` and `marketpulse-alerts` as two instances of one `ecs-service` module, ECR ×2, RDS SQL Server Express `db.t3.micro` (master password RDS-managed in Secrets Manager; task entrypoints compose `ConnectionStrings__MarketPulse` from injected `DB_*` vars so the password never sits in a task definition), **Amazon MQ for RabbitMQ `mq.t3.micro` — added although the roadmap's resource list omitted the broker**, since infrastructure that can't run the alerts pipeline fails the slice's own criterion (ADR-011 records the omission); private SPA bucket behind one dual-origin CloudFront distribution (`/api/*` + `/hubs/*` → ALB, same-origin so ADR-003's cookie posture and zero CORS carry over; SPA route fallback via a CloudFront Function, not `custom_error_response`, which would have rewritten API 403s). Also: the Alerts worker's first Dockerfile (+ entrypoint composition in both images, verified by container smoke test), a credential-free CI `infra` job (`fmt -check` + `validate` on both roots), `docs/cost-model.md` (~US$23/mo in free-tier year one, ~US$115/mo after; teardown-between-demos strategy), ADR-011. **What did not run: the live `terraform apply` rehearsal** — the building session's AWS SSO session was expired, so "reproducible from scratch" is asserted by construction and CI validation, not demonstrated. When credentials are refreshed the rehearsal is: clean apply from an empty account, CloudFront URL serves the SPA fallback, clean destroy. Slice 10 inherits three named seams: image push/SPA upload/service flip, the `RabbitMqOptions` TLS switch for AMQPS (5671-only broker, no TLS option in the app today), and the cloud OTel exporter (`Otel__OtlpEndpoint` deliberately unset) |
...
ledgers live in gitignored `.superpowers/` and do not survive the branch. Three landed as
fixes — two in 4b, while the messaging code they touch was still fresh, and the third in
slice 8:
...
- **Fixed in slice 8, with a caveat.** The transient requeue loop is bounded:
  `RabbitMqOptions.RetryLimit` (default 5) and `TransientRetry.RetryOrDeadLetterAsync`
  republish with an incremented `x-retry-count` and dead-letter at the cap,
  `marketpulse.notifications.redeliveries`/`.dead_letters` make it observable, and ADR-009 was
  amended to record the decision. Task 10's forced-failure rehearsal found the fix's reach is
  narrower than intended, though: it classifies on `DbUpdateException`, and a fully stopped SQL
  Server throws an unwrapped `SqlException` at connection-open instead — that fault shape skips
  the bounded-retry loop entirely and dead-letters on the very first delivery attempt, with no
  redelivery warning and no counter movement. No message is lost (the DLQ still catches it),
  but the visibility this bound exists to produce does not fire for that fault. Recorded here
  as an honest, non-blocking gap rather than folded into the "fixed" claim above it — see
  task-10-report.md; slice 11 owns the correction (widening the classification to cover
  connection-open `SqlException`, not just `DbUpdateException`).

Of the remaining two, both stay recorded here, non-blocking and unrelated to any slice's
current scope:
...
Four slices remain. Sizing assumes the ~8-task shape of slices 1–3; slices marked **may
...
Also owns slice 9's three named seams: pushing both images to ECR + uploading the SPA build
+ flipping the services live; the `RabbitMqOptions` TLS switch (Amazon MQ is AMQPS-only and
the app has no TLS option today); and the cloud OTel exporter decision (`Otel__OtlpEndpoint`
is deliberately unset in the task definitions). The live `terraform apply` rehearsal slice 9
couldn't run (expired credentials) should happen at this slice's start, before anything
deploys onto the result.

*Depends on:* 9 (done, apply rehearsal pending).

### 11 · Security hardening · phase 6
CSP with no `unsafe-inline` in production, security headers, `docs/THREAT-MODEL.md`,
dependency scanning, secrets handling review. Also owns the slice-8 rehearsal finding: widen
`AlertTriggeredConsumer`'s transient classification to catch connection-open `SqlException`,
not just `DbUpdateException`, so a fully unreachable database enters the bounded-retry loop
instead of dead-lettering on the first attempt.
...
| `docs/cost-model.md` | README docs index | **Resolved in 9** — see [cost-model.md](cost-model.md): free-tier vs. post-free-tier columns, the three deliberate cost decisions (no NAT, Express edition, single-AZ), the teardown-between-demos strategy |
